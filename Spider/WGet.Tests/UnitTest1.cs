...
public class UnitTest1
{
    ...
    string workspaceDir;
    string? mirrorUrl;
    string[]? commandLineUrls;

    private static  CancellationToken cancellation = new CancellationToken(false);
    private static List<string> knownTypes;
    private static Dictionary<string, string> mimeTypes;
    private static TaskFactory<string> testingTasks;
    ...
    public void Setup()
    {
        ...
        testingTasks = new TaskFactory<string>(cancellationToken: cancellation,
    ...
    public void EnvironmentVariables()
    {
        ...
        foreach (var item in Directory.GetFiles("/Users/owner/vsphere-k8s/.github/workflows/www.justice.gov"))
        {
    ...
    public async Task<string> GetContentType(string fileName)
    {
        ...
        {
            string headerType = await ScanFileForMimeType(fileName);
    ...
    private static async Task<string> ScanFileForMimeType(string fileName)
    {
        try
        {
            ...
            using (FileStream fs = new FileStream(fileName, FileMode.Open))
            {
                ...
                await fs.ReadAsync(buffer, 0, readLength);
            }

            UInt32 mimeType = default(UInt32);
            await FindMimeFromData(0, null, buffer, 256, null, 0, ref mimeType, 0);
    ...
    private static Task FindMimeFromData(int v1, object value1, byte[] buffer, int v2, object value2, int v3, ref uint mimeType, int v4)
    {
        var buffCopy = new byte[2048].AsMemory();
        return new Task(() =>
        {
            if (buffer.AsMemory().TryCopyTo(buffCopy))
                Debug.WriteLine(buffCopy.ToString());
            else 
